...
import {
  ...
  Wrench, ScanSearch, ChevronRight, Play, Monitor, FlaskConical,
} from "lucide-react";
...
export default function TopBar() {
  ...
  return (
    <div className="fixed top-0 left-0 md:left-64 right-0 z-40 bg-[#0f1117] border-b border-[#2d3748]">
      {/* Walkthrough banner */}
      {walkthroughActive && (
        <div className="px-3 py-2 flex flex-wrap items-center gap-2 border-b border-[#2d3748]"
          style={{ background: `${currentWTStep.color}12`, borderBottomColor: `${currentWTStep.color}30` }}>
          <div className="flex items-center gap-2 shrink-0">
            <FlaskConical size={12} style={{ color: currentWTStep.color }} />
          ...
          <div className="flex-1 min-w-0">
            ...
            <span className="text-xs text-[#94a3b8] hidden sm:inline">{currentWTStep.description}</span>
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <button onClick={() => { prevStep(); if (walkthroughStep > 0) router.push(walkthroughSteps[walkthroughStep - 1].page); }}
              ...
              className="px-2 py-1 rounded text-xs text-[#64748b] border border-[#2d3748] disabled:opacity-30">← </button>
            ...
            <button onClick={stopWalkthrough} className="text-[#64748b] text-xs px-1">✕</button>
          </div>
        </div>
      )}

      {/* Main bar */}
      <div className="flex items-center h-11 px-3 md:px-4 gap-2">
        {/* Spacer for mobile hamburger */}
        <div className="w-8 md:hidden shrink-0" />

        {/* Workflow ribbon — hidden on small, scrollable on md */}
        <div className="hidden sm:flex items-center gap-0 flex-1 overflow-x-auto scrollbar-none">
          {workflowSteps.map((step, i) => {
            ...
            return (
              <div key={step.href} className="flex items-center shrink-0">
                <Link href={step.href}
                  className={`flex items-center gap-1 px-2 md:px-3 py-1.5 rounded-lg text-[10px] md:text-[11px] font-medium transition-all ${
                    isActive ? "text-white" : "text-[#64748b] hover:text-[#94a3b8]"
                  }`}
                  style={{ background: isActive ? `${step.color}20` : "transparent", color: isActive ? step.color : undefined }}>
                  <Icon size={11} />
                  <span className="whitespace-nowrap hidden lg:inline">{step.label}</span>
                  {isActive && <span className="w-1 h-1 rounded-full" style={{ background: step.color }} />}
                </Link>
                {i < workflowSteps.length - 1 && (
                  <ChevronRight size={9} className="text-[#2d3748] shrink-0" />
                )}
              </div>
            );
          })}
        ...
        <div className="flex items-center gap-1.5 ml-auto shrink-0">
          {/* Impact strip — only xl */}
          <div className="hidden xl:flex items-center gap-2 px-2 py-1 rounded-lg bg-[#1a1f2e] border border-[#2d3748] text-[10px]">
            <span className="text-[#00d4aa] font-bold">⚡{impact.peakReduced}%</span>
            <span className="w-px h-3 bg-[#2d3748]" />
            <span className="text-[#10b981] font-bold">₹{impact.capexSaved}L</span>
            <span className="w-px h-3 bg-[#2d3748]" />
            <span className="text-[#f59e0b] font-bold">{impact.userSatisfaction}%</span>
          </div>

          {/* Mode toggle */}
          <div className="flex items-center rounded-lg border border-[#2d3748] overflow-hidden text-[10px] font-semibold">
            <button onClick={() => setMode("live")}
              className={`flex items-center gap-1 px-2 py-1.5 transition-colors ${mode === "live" ? "bg-[#10b981]/20 text-[#10b981]" : "text-[#64748b]"}`}>
            ...
            <button onClick={() => setMode("simulation")}
              className={`flex items-center gap-1 px-2 py-1.5 transition-colors ${mode === "simulation" ? "bg-[#6366f1]/20 text-[#6366f1]" : "text-[#64748b]"}`}>
              <Monitor size={10} />
          ...
          {!walkthroughActive && (
            <button onClick={() => { startWalkthrough(); router.push(walkthroughSteps[0].page); }}
              className="flex items-center gap-1 px-2 md:px-3 py-1.5 rounded-lg bg-[#6366f1] text-white text-[10px] md:text-[11px] font-semibold hover:bg-[#4f46e5] transition-colors">
              <Play size={10} />
